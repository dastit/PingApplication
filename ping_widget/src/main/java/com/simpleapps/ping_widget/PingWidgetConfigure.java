...
import com.simpleapps.pingme.TextInputAutoCompleteTextView;
...
public class PingWidgetConfigure extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(com.simpleapps.pingme.R.layout.widget_configure);

        hostNameWrapper = findViewById(com.simpleapps.pingme.R.id.wconfig_hostname_wrapper);
        hostName = findViewById(com.simpleapps.pingme.R.id.wconfig_hostName);
        hostName.requestFocus();

        shortHostNameWrapper = findViewById(
                com.simpleapps.pingme.R.id.wconfig_short_hostname_wrapper);
        shortHostName = findViewById(com.simpleapps.pingme.R.id.wconfig_short_hostName);

        updateRateWrapper = findViewById(
                com.simpleapps.pingme.R.id.wconfig_update_rate_wrapper);
        updateRate = findViewById(com.simpleapps.pingme.R.id.wconfig_update_rate);
        updateRateDim = findViewById(
                com.simpleapps.pingme.R.id.wconfig_update_rate_dimension);
        ...
        autoStartButton = findViewById(com.simpleapps.pingme.R.id.autostart_button);
        ...
        Button saveButton = findViewById(com.simpleapps.pingme.R.id.wconfig_save_button);

        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                                                                             com.simpleapps.pingme.R.array.update_rate_dims,
...
//                PingWidgetProvider.saveAlarmManager(alarmManager, pendingIntent);
    ...
    private boolean isCorrect() {
        ...
        if (mHostName.isEmpty()) {
            hostNameWrapper.setError(
                    getString(com.simpleapps.pingme.R.string.host_name_error));
            return false;
        }
    ...
    static void savePrefs(Context context, ArrayList<String> prefs, int widgetId) {
        SharedPreferences.Editor editor = context.getSharedPreferences(
                context.getString(com.simpleapps.pingme.R.string.preferences_name),
                MODE_PRIVATE).edit();
        editor.putString(context.getString(com.simpleapps.pingme.R.string.sp_name) + widgetId,
                         prefs.get(0));
        editor.putString(
                context.getString(com.simpleapps.pingme.R.string.sp_short_name) + widgetId,
                prefs.get(1));
        editor.putLong(context.getString(com.simpleapps.pingme.R.string.sp_rate) + widgetId,
    ...
    static String loadHostName(Context context, int widgetId) {
        ...
        return sharedPreferences.getString(
                context.getString(com.simpleapps.pingme.R.string.sp_name) + widgetId, "");
    }
    ...
    static long loadUpdateRate(Context context, int widgetId) {
        ...
        return sharedPreferences.getLong(
                context.getString(com.simpleapps.pingme.R.string.sp_rate) + widgetId,
                86400000);
    }
    ...
    static void deletePrefs(Context context, int widgetId) {
        ...
        editor.remove(context.getString(com.simpleapps.pingme.R.string.sp_short_name) +
                              widgetId);
        editor.remove(context.getString(com.simpleapps.pingme.R.string.sp_rate) + widgetId);
        editor.remove(context.getString(com.simpleapps.pingme.R.string.sp_name) + widgetId);
        editor.apply();
    }
